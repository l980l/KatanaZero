...
{	
	...
	if (nullptr == m_ParentUI)
	{
		...
		if (!m_Modal)
		{
			ImGui::Begin(strFullName.c_str(), &m_Active, m_ImGuiFlags);
		...
		else
		{
			...
			ImGui::OpenPopup(strFullName.c_str(), m_ImGuiFlags);
			if (ImGui::BeginPopupModal(strFullName.c_str(), &m_Active, m_ImGuiFlags))
			{
				render_update();
...
void UI::SetFlags(int _iFlags)
{
	m_ImGuiFlags = _iFlags;
}