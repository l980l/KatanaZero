...
int MaterialUI::render_update()
{
    ...
    ImGui::InputText("##MtrlNewName", szEmtpy, 30);
    ImGui::SameLine();

    if (ImGui::Button("##MtrlSaveButton", ImVec2(18.f, 18.f)))
    {
        string strTemp = szEmtpy;
        wstring wstrTemp = wstring(strTemp.begin(), strTemp.end());

        Ptr<CMaterial> pCopyMtrl = pMtrl->Clone();
        pCopyMtrl->SetEngineRes(false);
        pCopyMtrl->Save(L"material\\" + wstrTemp + L".mtrl");
    }
