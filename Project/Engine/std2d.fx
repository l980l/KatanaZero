...
#define bAnimLeft       g_int_1
#define LeftTop         g_vec2_0
#define Slice           g_vec2_1
#define Offset          g_vec2_2
#define BackSize        g_vec2_3
#define ArrangeColor    g_vec4_0
...
float4 PS_Std2DLight(VS_Light_OUT _in) : SV_Target
{
    ...
    if (g_btex_0)
    {
        if (bAnimUse)
        {
            if (bAnimLeft)
            {
                float2 newTexCoord = _in.vUV;
                // 좌우 반전 적용
                newTexCoord.x = abs(1.f - _in.vUV.x);

                _in.vUV.x = newTexCoord.x;
            }
                ...
                && LeftTop.y < vUV.y && vUV.y < LeftTop.y + Slice.y)
            {
                ...
                // 색상 적용.
                vOutColor.x *= ArrangeColor.x;
                vOutColor.y *= ArrangeColor.y;
                vOutColor.z *= ArrangeColor.z;
            }
            else
            {
