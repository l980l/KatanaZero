...
void CParticleSystem::Save(const wstring& _strRelativePath)
{
	...
	{
		wchar_t szStr[256] = {};
		wsprintf(szStr, L"ParticleSystem Save 실패, Error Number : %d", iErrNum);
...
void CParticleSystem::Load(const wstring& _strRelativePath)
{
	wstring strFilepath = CPathMgr::GetInst()->GetContentPath();
	strFilepath += _strRelativePath;
	...
	{
		/*wchar_t szStr[256] = {};
		wsprintf(szStr, L"ParticleSystem Load 실패, Error Number : %d", iErrNum);
		MessageBox(nullptr, szStr, L"파일 로딩 실패", MB_OK);*/
		return;
	}

	// ParticleSystem 키 값 저장
