...
CMonsterGruntScript::CMonsterGruntScript()
	: CScript((UINT)SCRIPT_TYPE::MONSTERGRUNTSCRIPT)
	, m_fSpeed(33.f)
	, m_iGround(0)
	, m_iGroundToFall(0)
	, m_iLWall(0)
...
void CMonsterGruntScript::SetDetect(int _iDetect)
{
...
void CMonsterGruntScript::Rewind(float Scale, bool Backward)
{
	m_RewindTime += DT;
	...
	if (Backward)
	{
		if (m_RewindTime >= 1.f / (60.f * Scale * m_RewindScale))
		{
			...
			MonsterGruntRewindData TempData = {};
			...
			{
			case 0:
				Animator2D()->Play(L"animation\\Grunt_walk.anim", true);
				break;
			case 1:
				Animator2D()->Play(L"animation\\Grunt_leavestair.anim", true);
				break;
			case 2:
				Animator2D()->Play(L"animation\\Grunt_run.anim", true);
				break;
			case 3:
				Animator2D()->Play(L"animation\\Grunt_turn.anim", true);
				break;
			case 4:
				Animator2D()->Play(L"animation\\Grunt_enterstair.anim", true);
				break;
			case 5:
				Animator2D()->Play(L"animation\\Grunt_hurtground.anim", true);
				break;
			case 6:
				Animator2D()->Play(L"animation\\Grunt_idle.anim", true);
				break;
			case 7:
				Animator2D()->Play(L"animation\\Grunt_attack.anim", true);
				break;
			case 8:
				Animator2D()->Play(L"animation\\Grunt_hurtground.anim", false);
				break;
			}
...
void CMonsterGruntScript::StartRewind()
{
	m_bRewind = true;
...
void CMonsterGruntScript::begin()
{
	...
	// 0 Grunt_walk 1 Grunt_leavestair 2 Grunt_run 3 Grunt_turn 4 Grunt_enterstair 5 Grunt_hurtground 6 Grunt_idle 7 Grunt_attack
	m_vecNextAnim[3] = 0;
	m_vecNextAnim[7] = 2;
}

void CMonsterGruntScript::tick()
{
	if (m_bRewind)
	{
		Rewind(0.1f, true);
	...
	else
	{
		...
		else if (m_iPlayerDetect == 1)
		{
			...
			else if (m_bAttackReady)
			{
				...
				// Effect_grunt_slash 프리팹 참조
				Ptr<CPrefab> pEffectPrefab = CResMgr::GetInst()->FindRes<CPrefab>(L"prefab\\Effect_grunt_slash.pref");
				Vec3 vEffectPos = Transform()->GetRelativePos();
				CGameObject* pCloneEffect = pEffectPrefab->Instantiate();
			...
			else
			{
				...
				if (PlayerScript->GetFloorLevel() == GetFloorLevel())
				{
					...
					if (TargetPos.x > m_NewMonsterPos.x)
					{
						...
						{
							Ptr<CSound> pSound = CResMgr::GetInst()->FindRes<CSound>(L"sound\\sound_generic_enemy_run_1.wav");
...
void CMonsterGruntScript::BeginOverlap(CCollider2D* _Other)
{
}

void CMonsterGruntScript::OnOverlap(CCollider2D* _Other)
{
}

void CMonsterGruntScript::EndOverlap(CCollider2D* _Other)
{
}

void CMonsterGruntScript::SaveToLevelFile(FILE* _File)
{
...
void CMonsterGruntScript::LoadFromLevelFile(FILE* _FILE)
{
